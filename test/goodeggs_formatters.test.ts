...
describe('goodeggs-formatters', function() {
  ...
  describe('formatDay', function() {
    ...
    describe('twoLetterDayOfTheWeek', () =>
      it('formats correctly', () => expect(f.formatDay('2017-02-01', 'twoLetterDayOfTheWeek')).toEqual('we'))
    );
  });

  it('formatDate', () => expect(f.formatDate('2014-3-8', 'mailChimpDate', clock.utc.tzid)).toEqual('03/08/2014'));
  ...
  describe('formatProductName', function() {
    ...
    it('falls back to product name', () => expect(f.formatProductName(product)).toEqual('Muffin of the Week'));
  ...
  describe('.formatDateRange', function() {
    ...
    it('formats a date range of different dates', function() {
      ...
      expect(f.formatDateRange(startAt, endAt, clock.pacific.tzid)).toEqual('Sunday, Mar 11, 9am - Tuesday, Mar 13, 9am');
    });

    it('formats a date range of the same day', function() {
      const startAt = clock.pacific('2012-03-11 09:00');
      const endAt = clock.pacific('2012-03-11 19:20');
      expect(f.formatDateRange(startAt, endAt, clock.pacific.tzid)).toEqual('Sunday, Mar 11, 9am - 7:20pm');
    });
    ...
    it('formats a date range object', function() {
      ...
      expect(f.formatDateRange(range, clock.pacific.tzid)).toEqual('Sunday, Mar 11, 9am - Tuesday, Mar 13, 9am');
      expect(f.formatDateRange(range, clock.pacific.tzid, {separator: ' to '})).toEqual('Sunday, Mar 11, 9am to Tuesday, Mar 13, 9am');
    });
  });
  ...
  describe('.formatDate', function() {
    ...
    describe('humanDate', () =>
      it('returns a full month name with ordinalized date', () => expect(f.formatDate(date, 'humanDate', clock.pacific.tzid)).toEqual("March 8th"))
    );
    ...
    describe('humanWeekday', function() {
      ...
      it('formats arbitrary weekdays', () => expect(f.formatDate(date, 'humanWeekday', clock.pacific.tzid)).toEqual('Thursday'));
    });

    describe('humanShortDay', () =>
      it('returns abbreviated month and ordinalized day', () => expect(f.formatDate(date, 'humanShortDay', clock.pacific.tzid)).toEqual('Thursday, Mar 8th'))
    );
    ...
    describe('humanShoppingDay', function() {
      ...
      it('formats arbitrary weekdays', () => expect(f.formatDate(date, 'humanShoppingDay', clock.pacific.tzid)).toEqual('Thursday 3/8'));
    });
    ...
    describe('humanShortShoppingDay', function() {
      ...
      it('formats today', function() {
        sandbox.stub(Date, 'now').returns(clock.pacific('2012-03-08 12:00'));
        expect(f.formatDate(date, 'humanShortShoppingDay', clock.pacific.tzid)).toEqual('today 3/8');
      });

      it('formats tomorrow', function() {
        sandbox.stub(Date, 'now').returns(clock.pacific('2012-03-07 12:00'));
        expect(f.formatDate(date, 'humanShortShoppingDay', clock.pacific.tzid)).toEqual('tomorrow 3/8');
      });

      it('formats another day', () => expect(f.formatDate(date, 'humanShortShoppingDay', clock.pacific.tzid)).toEqual('Thu 3/8'));
    });


    describe('shortShoppingDay', () =>
      it('returns a short shopping day', () => expect(f.formatDate(date, 'shortShoppingDay', clock.pacific.tzid)).toEqual('Thu 3/8'))
    );
    ...
    describe('orderCutoffDateTime', function() {
      it('formats time', function() {
        date = clock.pacific('2012-03-08 17:00');
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-08'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('5pm');
        clock.now.restore();
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-07'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('5pm tomorrow');
        clock.now.restore();
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-06'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('5pm Thursday 3/8');
        clock.now.restore();
      });
      ...
      it('special cases 12:00pm', function() {
        ...
        clock.now.restore();
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-07'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('noon tomorrow');
        clock.now.restore();
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-06'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('noon Thursday 3/8');
        clock.now.restore();
      });
      ...
      it('special cases 12:00am', function() {
        ...
        clock.now.restore();
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-07'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('midnight tomorrow');
        clock.now.restore();
        sandbox.stub(clock, 'now').returns(clock.pacific('2012-03-06'));
        expect(f.formatDate(date, 'orderCutoffDateTime', clock.pacific.tzid)).toEqual('midnight Thursday 3/8');
        clock.now.restore();
      });
    });
  });
  ...
  describe('formatLocation', () =>
    it('formats the full address', function() {
      ...
      expect(f.formatLocation(location, 'full')).toEqual('530 Hampshire Street, Suite 301, San Francisco, CA 94110');
    })
  );
});
