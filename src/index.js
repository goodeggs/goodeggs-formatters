...
const formatMoney = function(...args) {
  if (((args[1] != null ? args[1].wholeNumberPrecision : undefined) != null) && ((Math.round(args[0] * 100) % 100) === 0)) {
...
const formatCustomerName = ({firstName, lastName}) => _([firstName, lastName]).chain().compact().invoke('trim').value().join(" ");

const formatProductName = function(productOrLineItem, options) {
  if (options == null) { options = {}; }
  const {variant, excludeProductName, pickupDate} = options;
  if (productOrLineItem.get == null) { getto(productOrLineItem); }

  if (variant === 'stack') {
    return productOrLineItem.get('stackName');
  } else if ((typeof productOrLineItem.ofTheIsAvailableFor === 'function' ? productOrLineItem.ofTheIsAvailableFor({pickupDate}) : undefined) && (productOrLineItem.get('ofThe.name') != null)) {
...
const formatDeliveryWindow = (pickupWindow, tzid) => `${formatDate(pickupWindow.startAt, 'clockTime', tzid)}-${formatDate(pickupWindow.endAt, 'clockTime', tzid)}`;
